 ...
 * @returns Validated status or 'unknown' if invalid
 */
function validateBuildStatus(status: string): BuildStatus {
  const validStatuses: BuildStatus[] = ['success', 'failure', 'pending', 'unknown'];
  return validStatuses.includes(status as BuildStatus) ? status as BuildStatus : 'unknown';
}
...
function main(): void {
  try {
    const status = validateBuildStatus(process.argv[2] || 'unknown');
    const outputPath = process.argv[3] || join(__dirname, '..', 'badges', 'build.svg');
    ...
    console.error('Error generating build badge:', error instanceof Error ? error.message : 'Unknown error');
    process.exit(1);
  }
}
...
export { generateBadgeSVG, validateBuildStatus };