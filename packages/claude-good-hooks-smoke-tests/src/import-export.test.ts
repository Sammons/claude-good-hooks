...
import { runCLI, expectValidJSON } from './cli-utils.js'
...
describe('Claude Good Hooks CLI - Import/Export Cycle Tests', () => {
  const testDir = path.join(__dirname, '../temp-import-export-test')
  ...
  describe('Complete Import/Export Cycle', () => {
    test('should import, export, and verify configuration integrity', async () => {
      ...
      console.log(`✓ Verified ${originalHookCount.total} hooks across ${originalHookCount.events} events`)
    ...
    test('should handle JSON format import/export with all scopes', async () => {
      ...
      const jsonImportResult = await runCLI(['--json', 'import', configPath, '--scope', 'local', '--yes'])
      
      expect(jsonImportResult.success).toBe(true)
      
      // Export with JSON format
      const jsonExportResult = await runCLI(['--json', 'export', '--scope', 'local', '--output', reimportPath])
    ...
    test('should preserve all hook properties during import/export cycle', async () => {
      ...
      // Export it back
      const exportResult = await runCLI(['export', '--scope', 'global', '--output', edgeExportPath])
      expect(exportResult.success).toBe(true)
      ...
      const originalPreToolUse = edgeCaseConfig.settings.hooks.PreToolUse
    ...
    test('should handle import/export errors gracefully', async () => {
      ...
      expect(invalidImportResult.stderr || invalidImportResult.stdout).toMatch(/Failed to load configuration|File not found/i)
  ...
  describe('Import/Export Format Verification', () => {
    test('should support YAML export format', async () => {
      ...
      const yamlExportResult = await runCLI(['export', '--scope', 'project', '--format', 'yaml', '--output', yamlExportPath])
    ...
    test('should support template export format', async () => {
      ...
      const templateExportResult = await runCLI(['export', '--scope', 'project', '--format', 'template', '--output', templateExportPath])
    ...
    test('should support minified JSON export', async () => {
      ...
      const minifiedExportResult = await runCLI(['export', '--scope', 'project', '--minify', '--output', minifiedExportPath])
