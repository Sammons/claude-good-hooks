...
export function validateAndNormalizeSettings(
  ...
  if (!normalizedSettings.$schema) {
    normalizedSettings.$schema = 'https://github.com/sammons/claude-good-hooks/schemas/claude-settings.json';
  }
 ...
 */
export function validatePartialSettings(partialSettings: Partial<VersionedClaudeSettings>): SchemaValidationResult {
 ...
 */
export function performCustomValidation(settings: VersionedClaudeSettings): SchemaValidationError[] {
    ...
    for (let configIndex = 0; configIndex < configurations.length; configIndex++) {
      ...
      for (let hookIndex = 0; hookIndex < config.hooks.length; hookIndex++) {
        ...
        if (hook.timeout !== undefined) {
          ...
          if (hook.timeout > 3600000) { // 1 hour max
            errors.push({
              path: `${hookPath}.timeout`,
