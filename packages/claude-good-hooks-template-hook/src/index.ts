...
const templateHook: HookPlugin = {
  ...
  makeHook: (
    ...
    _context: { settingsDirectoryPath: string }
  ): NonNullable<ClaudeSettings['hooks']> => {
    // The _context provides information about the settings environment
    // _context.settingsDirectoryPath - absolute path to the .claude directory where settings.json is located
