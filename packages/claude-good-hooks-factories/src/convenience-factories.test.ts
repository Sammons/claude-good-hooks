...
describe('createFileWatcherHook', () => {
  ...
  it('should throw error for empty patterns', () => {
    expect(() => createFileWatcherHook([], 'echo test')).toThrow(
...
describe('createConditionalHook', () => {
  ...
  it('should create a conditional hook with only true command', () => {
    const hook = createConditionalHook('test -f package.json', 'npm run build');
  ...
  it('should create a conditional hook with custom event type', () => {
    const hook = createConditionalHook('test -f .env', 'echo "env found"', undefined, 'PreToolUse');
  ...
  it('should throw error for empty condition', () => {
    expect(() => createConditionalHook('', 'echo test')).toThrow(
      'Condition must be a non-empty string'
    );
  });

  it('should throw error for empty true command', () => {
    expect(() => createConditionalHook('test -f file', '')).toThrow(
      'True command must be a non-empty string'
    );
  });
});
