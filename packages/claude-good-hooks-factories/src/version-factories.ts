...
export function compareVersions(
  version1: string | HookVersion,
  version2: string | HookVersion
): number {
...
export function checkCompatibility(
  ...
  if (compatibility.minimumVersion && compareVersions(version, compatibility.minimumVersion) < 0) {
    return false;
  }

  // Check maximum version
  if (compatibility.maximumVersion && compareVersions(version, compatibility.maximumVersion) > 0) {
    return false;
  }
...
function satisfiesVersionConstraint(version: string, constraint: string): boolean {
  ...
  switch (operator) {
    ...
    case '=':
      return comparison === 0;
    case '^':
      return satisfiesCaretConstraint(version, constraintVersion);
    case '~':
      return satisfiesTildeConstraint(version, constraintVersion);
    default:
      return false;
  }
}
